    ...
    <section className={`${pageCard} mb-6`}>
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap sm:flex-nowrap rounded-xl p-2 -m-2">
        <div className="min-w-0 flex-1">
          ...
          <p className="text-xs text-gray-600 dark:text-slate-400 -mt-1">Total: {totalRetailers}</p>
        </div>
      </div>

      {/* Expanded Content - Only show when there are retailers or new rows */}
            ...
            <div className="flex items-center gap-2">
              {hasNewRows ? (
                <>
                  ...
                  <button
                    onClick={() => {
                      newRetailerRows.forEach(row => onRemoveNewRow(row.id));
                      onToggleAllRetailersSelection(false);
                    }}
                    className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg border border-slate-600 bg-slate-800/60 hover:bg-slate-700 hover:border-slate-500 text-slate-200 transition-all duration-200 flex items-center justify-center group"
      ...
      {totalRetailers === 0 && !hasNewRows && (
        <div className="pt-5 border-t border-gray-200 dark:border-slate-800 mt-4">
          <div className="text-center py-12">
            <div className="text-gray-600 dark:text-slate-400 text-lg mb-4">No retailers yet</div>
            <div className="text-slate-500 text-sm">
              Start building your retailer database by{" "}
              <button
                onClick={onAddNewRetailerRow}
