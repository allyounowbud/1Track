...
async function handleSearch(params: URLSearchParams) {
  try {
    ...
    // Make API call with correct parameter name 't' instead of 'api_key'
    const apiUrl = `https://www.pricecharting.com/api/products?q=${encodeURIComponent(cleanName)}&t=${PRICE_CHARTING_API_KEY}`
    ...
    // Log the first few products to see the structure
    if (data.products && data.products.length > 0) {
      console.log('First product structure:', JSON.stringify(data.products[0], null, 2))
      console.log('First 3 products:', data.products.slice(0, 3))
    }
    
    // Return the raw data with additional debugging info
    return json({
      success: true,
      data: data,
      ...
      debug: {
        ...
        firstProductKeys: data.products && data.products.length > 0 ? Object.keys(data.products[0]) : []
      }
    })
